...
#' )
barcode_ggheatmap_annie_v2.2 <- function(your_SE,
                                       plot_labels = NULL,
                                       n_clones = 10,
                                       ...
                                       return_table = FALSE,
                                       return_clones = FALSE,
                                       expanded_comparison = FALSE,
                                       comparison_group1 = NULL,
                                       comparison_group2 = NULL,
                                       return_biased_clone_num = FALSE,
                                       return_biased_clone_ID = FALSE) {
...
#################### START EDIT HERE ##########################
  ...
  if (expanded_comparison){
    ...
    group1_number <- vector(mode = "list", length = 0)
    for (i in 1:length(comparison_group1)){
      temp_number <- list(c(grep(comparison_group1[i], colnames(heatmap_data))))
      #print(temp_number)
      group1_number <- append(group1_number, temp_number)
      #print(group1_number)
    }
    #print("This is group1 number")
    #print(group1_number)
    #print(length(group1_number))
    group1_mean_all <- data.frame(matrix(NA, nrow = length(barcode_order), ncol = 0))
    for (i in 1:length(group1_number)){
      #print(comparison_group1[i])
      group1_number_element <- group1_number[[i]]
      if (length(group1_number_element) == 1){
        heatmap_tissue_temp <- heatmap_data[, group1_number_element]
        group1_mean_heatmap_temp <- data.frame(heatmap_tissue_temp)
        colnames(group1_mean_heatmap_temp) <- comparison_group1[i]
        rownames(group1_mean_heatmap_temp) <- barcode_order
        group1_mean_all <- cbind(group1_mean_all, group1_mean_heatmap_temp)
      } else{
        heatmap_tissue_temp <- heatmap_data[, group1_number_element]
        group1_mean_heatmap_temp <- data.frame(rowMeans(heatmap_tissue_temp))
        colnames(group1_mean_heatmap_temp) <- comparison_group1[i]
        rownames(group1_mean_heatmap_temp) <- barcode_order
        group1_mean_all <- cbind(group1_mean_all, group1_mean_heatmap_temp)
      }
    }
    #print(group1_mean_all)
    group2_number <- c()
    for (i in 1:length(comparison_group2)){
      temp_number <- grep(comparison_group2[i], colnames(heatmap_data))
      group2_number <- c(group2_number, temp_number)
    }

    #print(group2_number)

    if (length(group2_number) == 1){
      heatmap_tissue_temp <- heatmap_data[, group2_number]
      group2_mean_heatmap <- data.frame(heatmap_tissue_temp)
      colnames(group2_mean_heatmap) <- "Group1"
      rownames(group2_mean_heatmap) <- barcode_order
    } else{
      heatmap_tissue_temp <- heatmap_data[, group2_number]
      group2_mean_heatmap <- data.frame(rowMeans(heatmap_tissue_temp))
      colnames(group2_mean_heatmap) <- "Group1"
      rownames(group2_mean_heatmap) <- barcode_order
    }
    
    for (j in 1:length(group1_number)){
      #print(j)
      if (j == 1) {
        fold_change <- exp(group2_mean_heatmap - group1_mean_all[, j])
        #print(fold_change)
        clones_biased <- which(fold_change >= foldchange_threshold)
        #print(clones_biased)
      } else{
    ...
    clones_biased_table <- table(clones_biased)
    clones_biased_final <- as.numeric(names(which(clones_biased_table >= length(group1_number))))
    clones_biased_final_num <- length(clones_biased_final)
    #print(clones_biased_final)
    #clones_biased_df <- data.frame(barcode_order[clones_biased_final])
    
    #fold_change <- exp(group2_mean_heatmap - group1_mean_heatmap)
    #print(fold_change)
    #clones_biased <- which(fold_change >= foldchange_threshold)
    #print("Here are the biased clones")
    #print(clones_biased)
    clones_biased_barcode <- barcode_order[clones_biased_final]
    #print(clones_biased_barcode)
    #print(clones_biased_barcode)
    #print("this is the barcode orders")
    #print(barcode_order)

    clones_of_interest_info <- data.frame(as.integer(barcode_order %in% clones_biased_barcode))
    ...
    g1_heatmap <- g1_heatmap + ggplot2::theme(plot.margin = ggplot2::unit(c(5.5, 5.5, 5.5, 1), "pt"))
    cols <- c("0" = "grey79", "1" = "#782B9D", "2" = "#de7065ff", "3" = "#339933") #c("0" = "grey79", "1" = "mediumpurple1", "2" = "#7FC97F", "3" = "#FDC086")
    g6_clusters <- ggplot2::ggplot(clones_of_interest_info, ggplot2::aes(x = 1, y = .data$assignment, fill = factor(.data$value))) +
      ggplot2::geom_tile() +
      ggplot2::scale_x_continuous(expand = c(0, 0), labels = invisible_label, breaks = 1) +
      ggplot2::scale_y_discrete(expand = c(0, 0)) +
      ggplot2::theme(
    ...
    #print("hi")
    cowplot::plot_grid(g6_clusters, g1_heatmap, rel_widths = c(.2, 4), ncol = 2)
    if (dendro) {
      g6_clusters <- g6_clusters + ggplot2::theme(plot.margin = ggplot2::unit(c(5.5, 1, 5.5, 1), "pt"))
    }
    if (!is.null(tissue_specific_clones)) {
      g6_clusters <- g6_clusters + ggplot2::theme(plot.margin = ggplot2::unit(c(5.5, 1, 5.5, 5.5), "pt"))
    }
    if (!is.null(your_title)) {
      g6_clusters <- g6_clusters + ggplot2::ggtitle("")
    }
  }
  if (return_biased_clone_num && return_biased_clone_ID) {
    return(list("Biased_clone_num" = clones_biased_final_num, "Biased_clone_barcode" = clones_biased_barcode))
  } else if (return_biased_clone_ID) {
    return(clones_biased_barcode)
  } else if (return_biased_clone_num){
    return(clones_biased_final_num)
  }
  
  print(cowplot::plot_grid(g6_clusters, g1_heatmap, rel_widths = c(.2, 4), ncol = 2))
  ...
  #   g1_heatmap
  # } else if (clusters == 0 & !dendro & expanded_comparison & is.null(tissue_specific_clones) & is.null(clones_of_interest)) {
  #   cowplot::plot_grid(g6_clusters, g1_heatmap, rel_widths = c(.2, 4), ncol = 2)
  # } else if (clusters == 0 & !dendro & !expanded_comparison & !is.null(tissue_specific_clones)){
  #   cowplot::plot_grid(g5, g1_heatmap, rel_widths = c(.2, 4), ncol = 2)
  # } else if (clusters == 0 & !dendro & expanded_comparison & !is.null(tissue_specific_clones)){
  #   cowplot::plot_grid(g6_clusters, g5, g1_heatmap, rel_widths = c(.2, .2, 4), ncol = 3)
  # }
  # } else if (clusters == 0 & !dendro & !is.null(clones_of_interest) & is.null(tissue_specific_clones)) {
  #   cowplot::plot_grid(g4_clusters, g1_heatmap, rel_widths = c(.2, 4), ncol = 2)
  # } else if (clusters == 0 & !dendro & is.null(clones_of_interest) & !is.null(tissue_specific_clones)){
