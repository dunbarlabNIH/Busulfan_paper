...
source("/Volumes/dirgroup/TSCBB/LAB/Annie/6_Tissue_NK_Manuscript/scripts/barcode_ggheatmap_annie_v2.1.R")
source("/Volumes/dirgroup/TSCBB/LAB/Annie/6_Tissue_NK_Manuscript/scripts/barcode_ggheatmap_annie_v2.2.R")

# input information about monkey and samples
monkey_ID <- "H84D"
condition_type <- "Busulfan"
analysis_type <- "NK_bias"
process_date <- 20220510
...
metadata_df <- read.delim(file = file.path(read_dir,paste(monkey_ID, "_",process_date,"_NElib_metadata.txt", sep="")),
                          sep = "\t")
...
plot_labels_old <- paste( paste(metadata_df[sample_index, ]$Month_timepoint, "m", sep = ""), metadata_df[sample_index, ]$Cell.Type)
plot_labels <- paste( paste(metadata_subset$Month_timepoint, "m", sep = ""), metadata_subset$Cell.Type)
...
for (i in 1:length(timepoints_unique)){
  ...
  CD16_index <- which(SE_subset$Cell.Type == "CD16+")
  
  SE_subset_proportions_assay <- SummarizedExperiment::assays(SE_subset)[["proportions"]]
  colnames(SE_subset_proportions_assay) <- SE_subset$Cell.Type
  ...
  for (j in 1:length(SE_subset$Cell.Type)){
    
    if (SE_subset$Cell.Type[j] != "CD16+"){
      #print(j)
  ...
  biased_clones_final_barcode <- names(which(biased_clones_table >= length(unique(SE_subset$Cell.Type)) - 1))
