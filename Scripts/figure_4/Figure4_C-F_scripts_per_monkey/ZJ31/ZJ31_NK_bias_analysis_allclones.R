...
plot_labels_old <- paste( paste(metadata_df[sample_index, ]$months, "m", sep = ""), metadata_df[sample_index, ]$simple_cell_type)
plot_labels <- paste( paste(metadata_subset$months, "m", sep = ""), metadata_subset$simple_cell_type)
print(sum(plot_labels_old == plot_labels)) # this number should equal to the number of samples being analyzed 

timepoints_unique <- unique(metadata_subset$months)

timepoint_index_list <- list()

for (i in 1:length(timepoints_unique)){
  timepoint_temp <- timepoints_unique[i]
  timepoint_indices <- which(metadata_subset$months == timepoint_temp)
